(async () => {
	// Note to this project
	// - Don't be lazy to set-up development, make your code modular
	// - I need to create a better architecture for rendering
	// - I need to start creating boilerplate functions

	const browser = chrome; // Remove if Firefox
	...
	let viewMonth = date.currentMonth;
	let viewDay = date.currentDay;
	let minYear;
	...
	updateDayStat(userData.tasks, viewYear, viewMonth, viewDay);
	...
	async function initListeners() {
		...
		openAsTab.setAttribute("href", `${browser.runtime.getURL("html/newtab.html")}`);
		const dayStatDate = document.getElementById("daystat-date");
		dayStatDate.textContent = dateFormat(new Date());
		...
		toggleList.addEventListener("click", async () => {
			const listMain = document.querySelector(".daystat-container");
		...
		prevDate.addEventListener("click", async () => {
			const { tasks } = await local.get("tasks");
			const latestDate = new Date(viewYear, viewMonth, 0);
			if (viewYear < minYear) return;

			if (viewDay <= 1) {
				viewDay = latestDate.getDate();
				if (viewMonth <= 0) {
					viewMonth = 11;
					viewYear -= 1;
				} else {
			...
			updateDayStat(tasks, viewYear, viewMonth, viewDay);
		});

		nextDate.addEventListener("click", async () => {
			const { tasks } = await local.get("tasks");
			const latestDate = new Date(viewYear, viewMonth + 1, 0);

			if (viewDay >= latestDate.getDate()) {
				viewDay = 1;
				if (viewMonth >= 11) {
					viewMonth = 0;
					viewYear += 1;
	...
	function updateDayStat(tasks, year, month, day) {
